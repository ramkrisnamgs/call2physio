...
export default function UpcomingAppointments() {
  ...
  if (error) {
    return <div className="text-red-500">Failed to load appointments</div>;
  }
    ...
    <div className="bg-white p-6 rounded-2xl shadow-lg border relative overflow-hidden">
      ...
      <ul className="space-y-3">
        {upcoming?.map((appt) => (
          <li
            key={appt.id}
            className="flex items-center justify-between bg-[#f0fdfd] rounded-lg px-4 py-2 hover:bg-[#e0f7fa] transition-colors border border-transparent hover:border-[#35B6B4] shadow-sm"
          >
